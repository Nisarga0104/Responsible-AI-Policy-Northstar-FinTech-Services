...
> **Status: post-benchmarking draft.** Benchmarked against Microsoft's Responsible AI Standard v2
> and IBM's Trust framework (see `../01_Research/benchmarking_notes.md`); Section 3 and Section 6
> now state explicitly that deployment requires meeting every relevant principle (a gate, not a
> checklist), matching how Microsoft's Standard treats its goals. This version is content-final.
> `../03_Final/` applies only formatting polish on top of this text -- no further factual changes.
